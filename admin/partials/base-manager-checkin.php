<?php
/**
 * Base Manager - Check-In Admin Page
 *
    ...
    <div class="yolo-bm-content">
        <div class="card" style="background: #fff; border: 1px solid #ccd0d4; box-shadow: 0 1px 1px rgba(0,0,0,.04); margin-top: 20px;">
            <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; padding: 15px; background: #f8f9fa; border-bottom: 1px solid #dee2e6;">
                <h2 style="margin: 0;"><span class="dashicons dashicons-yes-alt"></span> Check-In Management</h2>
                <button class="button button-primary" id="new-checkin-btn">
                    <span class="dashicons dashicons-plus-alt" style="vertical-align: middle;"></span> New Check-In
                </button>
            </div>
            <div class="card-body" style="padding: 20px;">
                
                <!-- Check-In Form (Hidden by default) -->
                <div id="checkin-form-container" style="display: none; margin-bottom: 30px; padding: 20px; background: #f9f9f9; border: 1px solid #ddd; border-radius: 4px;">
                    <h3>New Check-In</h3>
                    <form id="checkin-form">
                        <table class="form-table">
                            <tr>
                                <th scope="row"><label for="checkin-booking-select">Select Booking *</label></th>
                                <td>
                                    <select id="checkin-booking-select" class="regular-text" required>
                                        <option value="">Choose booking...</option>
                                    </select>
                                </td>
                            </tr>
                            <tr>
                                <th scope="row"><label for="checkin-yacht-select">Select Yacht *</label></th>
                                <td>
                                    <select id="checkin-yacht-select" class="regular-text" required>
                        ...
                        <div id="checkin-checklist-container" style="margin: 20px 0;"></div>
                        ...
                        <div class="signature-pad-container" style="border: 2px solid #ddd; border-radius: 4px; background: #fff; padding: 10px; margin-bottom: 20px;">
                            <canvas id="checkin-signature-pad" width="600" height="200" style="border: 1px solid #ccc; cursor: crosshair; display: block;"></canvas>
                            <button type="button" class="button" id="clear-checkin-signature" style="margin-top: 10px;">
                                <span class="dashicons dashicons-image-rotate" style="vertical-align: middle;"></span> Clear Signature
                            </button>
                        </div>
                        
                        <div class="form-actions">
                            <button type="button" class="button button-primary" id="complete-checkin-btn">
                                <span class="dashicons dashicons-yes" style="vertical-align: middle;"></span> Complete Check-In
                            </button>
                            <button type="button" class="button button-primary" id="save-checkin-pdf-btn">
                                <span class="dashicons dashicons-pdf" style="vertical-align: middle;"></span> Save PDF
                            </button>
                            <button type="button" class="button" id="send-checkin-guest-btn">
                                <span class="dashicons dashicons-email" style="vertical-align: middle;"></span> Send to Guest
                            </button>
                            <button type="button" class="button" id="cancel-checkin-btn">Cancel</button>
                        </div>
                    </form>
                </div>
...
jQuery(document).ready(function($) {
    ...
    $('#complete-checkin-btn').on('click', function() {
        ...
        const formData = {
            ...
            booking_id: $('#checkin-booking-select').val(),
            yacht_id: $('#checkin-yacht-select').val(),
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    alert('Error: ' + (response.data || 'Failed to save check-in'));
                }
            },
            error: function() {
    ...
    function loadCheckins() {
        ...
        $('#checkin-tbody').html('<tr><td colspan="5" style="text-align: center; color: #666;">No check-ins yet</td></tr>');
    }
});
</script>
