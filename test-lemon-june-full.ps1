...
# Add flexibility and productName parameters (as used in sync)
$url = "https://www.booking-manager.com/api/v2/offers?companyId=$companyId&dateFrom=$dateFrom&dateTo=$dateTo&flexibility=6&productName=bareboat"
$headers = @{
    'Authorization' = $apiKey
    'Accept' = 'application/json'
}

Write-Host "=== Lemon Yacht Availability Check - Full June 2026 ===" -ForegroundColor Cyan
Write-Host "Yacht: Lemon (ID: $yachtId)"
Write-Host "Date Range: June 1-30, 2026 (Full month)"
...
try {
    ...
    if ($lemonOffers.Count -gt 0) {
        Write-Host ""
        Write-Host "✅ AVAILABILITY: Lemon has offers in June 2026" -ForegroundColor Green
        Write-Host "Found $($lemonOffers.Count) offer(s) for Lemon"
        Write-Host ""
        
        # Group by week
        $week1Offers = $lemonOffers | Where-Object {
            $dateFrom = [DateTime]::Parse($_.dateFrom)
            $dateFrom -ge [DateTime]::Parse('2026-06-01') -and $dateFrom -le [DateTime]::Parse('2026-06-08')
        }
        
        if ($week1Offers.Count -gt 0) {
            Write-Host "✅ FIRST WEEK (June 1-8, 2026): AVAILABLE" -ForegroundColor Green
            Write-Host "Found $($week1Offers.Count) offer(s) for first week"
            Write-Host ""
        } else {
            Write-Host "❌ FIRST WEEK (June 1-8, 2026): NOT AVAILABLE" -ForegroundColor Red
            Write-Host ""
        }
        
        Write-Host "All Lemon offers in June 2026:"
        ...
        Write-Host ""
        Write-Host "❌ AVAILABILITY: Lemon does NOT appear to have any offers in June 2026" -ForegroundColor Red
        ...
        if ($data.Count -gt 0) {
            Write-Host ""
            Write-Host "Sample yacht names from results (to verify API is working):"
            $data | Select-Object -First 10 yacht | ForEach-Object {
                Write-Host "  - $($_.yacht)"
            }
        }
    }
