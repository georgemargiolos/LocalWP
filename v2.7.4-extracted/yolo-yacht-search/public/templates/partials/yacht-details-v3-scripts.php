...
function selectWeek(button) {
    ...
    // Remove old deposit info so it gets recreated with new price
    const depositInfo = document.getElementById('depositInfo');
    if (depositInfo) {
        depositInfo.remove();
    }
    
    // Recalculate deposit for the newly selected week
...
function updatePriceDisplayWithDeposit() {
    ...
    // Check if there's a live price from custom date selection
    if (window.yoloLivePrice && window.yoloLivePrice.price) {
