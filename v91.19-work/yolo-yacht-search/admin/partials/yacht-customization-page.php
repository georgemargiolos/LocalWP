...
// v91.20: Get company names from database (synced from API)
$db_instance = new YOLO_YS_Database();
$db_company_names = $db_instance->get_all_companies();

foreach ($friend_company_ids as $company_id) {
    // Get company name from database, fallback to "Company {id}"
    $company_name = isset($db_company_names[intval($company_id)]) 
        ? $db_company_names[intval($company_id)] 
        : "Company $company_id";
                            ...
                            <div class="media-item" data-id="<?php echo esc_attr($media->id); ?>" data-type="<?php echo esc_attr($media->media_type); ?>"
                                ...
                                <div style="padding: 5px; font-size: 11px; background: #f8f9fa;">
                                    ...
                                    <button type="button" class="delete-media-btn" data-id="<?php echo esc_attr($media->id); ?>" 
                                            style="float: right; background: none; border: none; color: #dc3545; cursor: pointer; padding: 8px; margin: -8px; touch-action: manipulation; -webkit-tap-highlight-color: transparent;">
...
jQuery(document).ready(function($) {
    ...
    $('#add-video-btn').on('click', function() {
        ...
        if (videoUrl) {
            ...
            var match = videoUrl.match(/(?:(?:www\.)?youtube\.com\/watch\?v=|youtu\.be\/|(?:www\.)?youtube\.com\/embed\/)([^&\s]+)/);
            if (match) {
                videoId = match[1];
            }
    ...
    // Delete media item (v91.10: added touchend for mobile support)
    $(document).on('click touchend', '.delete-media-btn', function(e) {
        e.preventDefault();
        e.stopPropagation();
