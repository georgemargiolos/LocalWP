...
function yolo_ys_format_boat_result($row, $details_page_url) {
    ...
    if ($use_custom_media) {
        // v91.7: Get first custom IMAGE (not video) for yacht card
        ...
        // If custom media is enabled but no images found (only videos), fall through to API images
    }
    
    // Fallback: Extract from raw_data if no image in database or custom media had no images
    if (empty($primary_image) && !empty($row->raw_data)) {
        $raw_data = json_decode($row->raw_data, true);
        if (!empty($raw_data['images'])) {
            foreach ($raw_data['images'] as $img) {
