...
class YOLO_YS_Booking_Manager_API {
    ...
    public function get_live_price($yacht_id, $date_from, $date_to) {
        ...
        if ($result['success']) {
            ...
            // FIX v60.6.14: Check for 'value' wrapper FIRST - this is the actual format!
            if (isset($result['data']['value']) && is_array($result['data']['value'])) {
                // Format: { "value": [...], "Count": N } - ACTUAL Booking Manager format!
                $offers = $result['data']['value'];
            } elseif (isset($result['data']['offers']) && is_array($result['data']['offers'])) {
                // Format: { "offers": [...] }
