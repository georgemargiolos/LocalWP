...
function yolo_show_booking_confirmation($booking) {
    ...
    // FIX v60.6.14: Track Purchase event for Facebook Pixel with RETRY LOGIC
    // PixelYourSite loads fbq asynchronously, so we need to wait for it
    (function() {
        var purchaseEventId = window.fbPurchaseEventId || 'purchase_<?php echo esc_js($booking_id_safe); ?>_<?php echo time(); ?>';
        var purchaseData = {
        ...
        var maxRetries = 20; // 10 seconds total (20 x 500ms)
        var retryCount = 0;
        
        function trackFBPurchase() {
            if (typeof fbq === 'function') {
                fbq('track', 'Purchase', purchaseData, { eventID: purchaseEventId });
                console.log('YOLO Analytics: Purchase event tracked (Facebook Pixel) with event_id:', purchaseEventId);
                return;
            }
            
            retryCount++;
            if (retryCount < maxRetries) {
                console.log('YOLO Analytics: Waiting for Facebook Pixel to load... (' + retryCount + '/' + maxRetries + ')');
                setTimeout(trackFBPurchase, 500);
            } else {
                console.warn('YOLO Analytics: Facebook Pixel not loaded after 10s, Purchase event not tracked client-side');
            }
        }
        
        // Start tracking (with retry logic for async fbq loading)
        trackFBPurchase();
    })();
    </script>
    <?php
