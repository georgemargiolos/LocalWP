 ...
 * ✓ v60.6.10: Use json_encode for all JS string outputs to prevent syntax errors
 *            - esc_js() doesn't handle all edge cases (newlines, unicode)
...
// CRITICAL: Clean yacht name - remove any characters that could break JavaScript
// Some yacht names have special Unicode characters, newlines, or quotes
$yacht_name_safe = '';
if ($yacht && isset($yacht->name)) {
    $yacht_name_safe = $yacht->name;
    // Remove any newlines, carriage returns, and normalize whitespace
...
// CRITICAL: Must be on window object so yolo-analytics.js can access it
...
function showCustomDatesModal(dateFrom, dateTo) {
    // Safe yacht name for use in JavaScript template literals
    const yachtNameForMessage = <?php echo json_encode($yacht_name_safe); ?>;
        ...
        <div id="customDatesModal" class="yolo-quote-modal">
            <div class="yolo-quote-modal-content">
                ...
                <form id="customDatesForm" class="mt-3">
                    ...
                    <textarea name="message" rows="4" class="form-control mb-2" placeholder="Message">I would like to charter ${yachtNameForMessage} from ${dateFrom} to ${dateTo}. Please contact me with availability and pricing.</textarea>
                    <input type="hidden" name="yacht_id" value="${yachtIdForModal}">
                    <input type="hidden" name="yacht_name" value="${yachtNameForMessage}">
...
function bookNow() {
    ...
    // v60.6.10: Use json_encode for all values to prevent JS syntax errors
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ============================================
    // Safe variables for analytics tracking (using json_encode to prevent JS syntax errors)
    const yachtIdForTracking = <?php echo json_encode($yacht_id_safe); ?>;
    const yachtNameForTracking = <?php echo json_encode($yacht_name_safe); ?>;
                    ...
                    if (data.data && data.data.event_id) {
                        if (typeof YoloAnalytics !== 'undefined') {
                            YoloAnalytics.trackLead({
                                yacht_id: yachtIdForTracking,
                                yacht_name: yachtNameForTracking,
                                value: 0
                            }, data.data.event_id);
                        }
                    }
