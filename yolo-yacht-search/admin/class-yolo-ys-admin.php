...
class YOLO_YS_Admin {
    ...
    /**
     * v91.20: AJAX handler to sync all companies from API
     * Fetches all companies from Booking Manager API and stores in database
     */
    public function ajax_sync_companies() {
        check_ajax_referer('yolo_ys_admin_nonce', 'nonce');
        ...
        try {
            ...
            if (empty($companies)) {
                wp_send_json_error(array('message' => 'No companies returned from API'));
                return;
            }
            
            $db = new YOLO_YS_Database();
            $result = $db->sync_companies($companies);
