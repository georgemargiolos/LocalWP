...
class YOLO_YS_Progressive_Sync {
    ...
    public function get_all_company_ids() {
        ...
        $friend_companies = get_option('yolo_ys_friend_companies', '');
    ...
    public function init_yacht_sync($selected_companies = null) {
        ...
        $friend_companies_setting = get_option('yolo_ys_friend_companies', '');
    ...
    public function sync_next_image_batch() {
        ...
        try {
            ...
            // If first batch for this yacht, clear old images from DB and calculate primary
            if ($is_first_batch) {
                $this->db->clear_yacht_images($yacht_id);
                // v91.20: Calculate primary image index once for all batches
                $state['primary_image_index'] = $this->db->find_best_primary_image($all_images);
            }
            
            // Get the cached primary index (or default to 0)
            $primary_index = isset($state['primary_image_index']) ? $state['primary_image_index'] : 0;
            ...
            foreach ($batch_images as $batch_idx => $image) {
                ...
                // v91.20: Pass is_primary based on smart detection
                $is_primary = ($actual_index === $primary_index);
                $result = $this->db->download_and_store_single_image($yacht_id, $image, $actual_index, $is_primary);
                if ($result) {
                    $images_downloaded++;
                    $state['synced_images']++;
                }
