...
class YOLO_YS_Stripe {
     ...
     * @param float $included_extras Amount of extras included in price
     * @param float $extras_at_base Amount of extras payable at base
     * @param string $extras_details JSON string of extras details
     * @return array Session data with session_id
     */
    public function create_checkout_session($yacht_id, $yacht_name, $date_from, $date_to, $total_price, $currency = 'EUR', $customer_first_name = '', $customer_last_name = '', $customer_email = '', $customer_phone = '', $included_extras = 0, $extras_at_base = 0, $extras_details = '[]') {
        try {
            $this->init_stripe();
            ...
            // Build description with extras info
            $description = sprintf(
            ...
            // Add extras information to description
            if ($included_extras > 0) {
                $description .= sprintf(' | Obligatory extras (%s) are INCLUDED in this price.', 
                    YOLO_YS_Price_Formatter::format_price($included_extras, $currency));
            }
            if ($extras_at_base > 0) {
                $description .= sprintf(' | Obligatory extras (%s) are PAYABLE AT THE BASE.',
                    YOLO_YS_Price_Formatter::format_price($extras_at_base, $currency));
            }
