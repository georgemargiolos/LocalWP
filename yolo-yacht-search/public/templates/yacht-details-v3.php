...
if ($use_custom_media) {
    ...
    // v91.20: Order by is_primary DESC first, then sort_order ASC
    // This ensures the primary image is always first in the gallery
    $images_table = $wpdb->prefix . 'yolo_yacht_images';
    $images = $wpdb->get_results($wpdb->prepare(
        "SELECT * FROM $images_table WHERE yacht_id = %s ORDER BY is_primary DESC, sort_order ASC",
        $yacht_id
    ));
    $videos = array();
}
        ...
        <h1 class="yacht-name">
            ...
            <?php if ($yacht->home_base): ?>
                ...
                <span class="location" onclick="document.querySelector('.yacht-map-section h3')?.scrollIntoView({behavior: 'smooth'});">
                    <i class="fa-duotone fa-location-dot" style="color: #dc2626;"></i> <?php echo esc_html($yacht->home_base); ?><?php 
                    // v85.0: Show airport info in same line
                    $airport_info = yolo_ys_get_nearest_airport($yacht->home_base);
                    if ($airport_info): 
                    ?> <span class="yolo-ys-location-separator"> | </span><i class="fa-duotone fa-plane" style="color: #3b82f6;"></i> <?php echo esc_html($airport_info[2]); ?>km from <?php echo esc_html($airport_info[1]); ?> - <?php echo esc_html($airport_info[0]); ?><?php endif; ?>
                </span>
            <?php endif; ?>
        </h1>
    </div>
                        ...
                        <div class="swiper-wrapper">
                            ...
                            if ($use_custom_media && !empty($media_items)): 
                                foreach ($media_items as $index => $item): 
                                    if ($item->media_type === 'video'):
                                        // v91.10: Extract YouTube video ID
                                        $video_url = trim($item->media_url);
                                        $video_id = null;
                                        
                                        // Check if it's already a YouTube video ID (11 chars, alphanumeric with - and _)
                                        if (preg_match('/^[a-zA-Z0-9_-]{11}$/', $video_url)) {
                                            $video_id = $video_url;
                                        } 
                                        // Try to extract from full YouTube URL (various formats)
                                        elseif (preg_match('/(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/', $video_url, $matches)) {
                                            $video_id = $matches[1];
                                        }
